...
/**
 * @see https://gist.github.com/colinhacks/074a8509cd506fdc1f96cad27cc20c77
 * @example
 * ```ts
 * type User = {
 *   name: string;
 ...
 * };
 *
 * // changing any method here results in a TypeError
 * export const User: toZod<User> = z.object({
 *   name: z.string().min(5), // extra validations are fine
...
export type toZod<T> = {
  ...
  date: z.ZodDate
  // made a slight modification over the gist due to type errors
  object: z.ZodObject<{ [k in keyof T]: toZod<T[k]> }, any>
  rest: never
}[zodKey<T>]
